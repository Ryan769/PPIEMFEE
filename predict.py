...
def load_model(pred_config, args, device):
    ...
    try:
        model = MyModel(pred_config.model_dir, args=args)
        model.to(device)
        model.eval()
...
def convert_input_file_to_tensor_dataset(
    ...
    char_dict = json.load(open(args.char_dict_path, encoding='utf-8'))
    pos_dict = json.load(open(args.pos_dict_path, encoding='utf-8'))
    ...
    with open(pred_config.input_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            for word in tokens:
                ...
                for char in word:
                    if char.lower() in char_dict:
                        ...
                        char_id.append(0)
                if len(char_id) > args.max_character_len:
                    char_id = char_id[:args.max_character_len]
                while len(char_id) < args.max_character_len:
                    char_id.append(0)
            ...
            char_ids = char_ids + ([[0] * args.max_character_len] * padding_length)
            pos_ids = pos_ids + ([0] * padding_length)
    ...
    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_char_ids, all_pos_ids, all_e1_mask, all_e2_mask)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", default="./output", type=str, help="Path to save, load model")
